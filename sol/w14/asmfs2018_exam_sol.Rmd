...
SNP4 <- c(1,1,1)
SNP5 <- c(0,0,-1)
tbl_snp_ge_data <- tibble::tibble(Tier = 1:n_anz_snp_tiere, 
                                     Gewicht = Gewicht,
                                     SNP1 = SNP1,
                                     SNP2 = SNP2,
                                     SNP3 = SNP3,
...
SNP5 <- c(0,0,-1)
sex <- c("F", "M","F")
tbl_sex_snp_ge_data <- tibble::tibble(Tier = 1:n_anz_snp_tiere, 
                                     Gewicht = Gewicht,
                                     Geschlecht = sex,
                                     SNP1 = SNP1,
                                     SNP2 = SNP2,
...
vec_y <- tbl_sex_snp_ge_data$Gewicht
cat("$$\n")
cat("y = \\left[")
...
* $b$ der Vektor der unbekannten fixen Effekte des Geschlechts mit 

```{r, echo=FALSE, results='asis'}
vec_b <- c("b_F", "b_M")
cat("$$\n")
cat("b = \\left[")
cat(paste0(rmddochelper::sConvertVectorToLaTexArray(pvec_avector = vec_b), collapse = "\n"), "\n")
cat("\\right]")
cat("$$\n")
```
...
```{r, echo=FALSE, results='asis'}
n_nr_sex_level <- nlevels(as.factor(tbl_sex_snp_ge_data$Geschlecht))
mat_x <- matrix(data = c(1, 0,
                         0, 1,
                         1, 0), ncol = n_nr_sex_level, byrow = TRUE)
cat("$$\n")
cat("X = \\left[")
...
* $q$ der Vektor der unbekannten zufälligen SNP-Effekte

```{r, echo=FALSE, results='asis'}
n_nr_snp <- sum(substr(colnames(tbl_sex_snp_ge_data), 1, 3) == "SNP")
vec_q <- rmddochelper::vecGetVecElem(psBaseElement = "q",pnVecLen = n_nr_snp, psResult = "latex")
cat("$$\n")
cat("q = \\left[")
cat(paste0(rmddochelper::sConvertVectorToLaTexArray(pvec_avector = vec_q), collapse = "\n"), "\n")
cat("\\right]")
cat("$$\n")
```
...
```{r, echo=FALSE, results='asis'}
mat_w <- matrix(data = c(1, 0,  1, 1,  0,
                         1, 1,  0, 1,  0,
                        -1, 0, -1, 1, -1), ncol = n_nr_snp, byrow = TRUE)
cat("$$\n")
cat("W = \\left[")
cat(paste0(rmddochelper::sConvertMatrixToLaTexArray(pmatAMatrix = mat_w, pnDigits = 0), collapse = "\n"), "\n")
cat("\\right]")
cat("$$\n")
```

* $e$ der Vektor der unbekannten zufälligen Fehlerterme

```{r, echo=FALSE, results='asis'}
n_nr_obs <- nrow(tbl_sex_snp_ge_data)
vec_e <- rmddochelper::vecGetVecElem(psBaseElement = "e", pnVecLen = n_nr_obs, psResult = "latex")
cat("$$\n")
cat("e = \\left[")
cat(paste0(rmddochelper::sConvertVectorToLaTexArray(pvec_avector = vec_e), collapse = "\n"), "\n")
cat("\\right]")
cat("$$\n")
```
...
Im GBLUP Model werden die genomischen SNP-Effekte pro Tier in einem zufälligen Effekt zusammengefasst. Das Modell lautet somit
...
* $g$ der Vektor der unbekannten zufälligen genomischen Zuchtwert

```{r, echo=FALSE, results='asis'}
n_nr_obs <- nrow(tbl_sex_snp_ge_data)
vec_g <- rmddochelper::vecGetVecElem(psBaseElement = "g",pnVecLen = n_nr_obs, psResult = "latex")
cat("$$\n")
cat("g = \\left[")
cat(paste0(rmddochelper::sConvertVectorToLaTexArray(pvec_avector = vec_g), collapse = "\n"), "\n")
cat("\\right]")
cat("$$\n")
```
...
$$\hat{g}_4 = -\left( G^{22}\right)^{-1}G^{21}\hat{g}_1$$
...
```{r GibbsSamplerEst, echo=FALSE, results='markup'}
### # Matrix X as incidence matrix for beta0 and beta1
X <- cbind(1,dfDataRead$x)
### # y as vector of observations
y <- dfDataRead$y
...
```{r GibbsSamplerSol, echo=FALSE, results='markup'}
### # Matrix X as incidence matrix for beta0 and beta1
X <- cbind(1,dfDataRead$x)
### # y as vector of observations
y <- dfDataRead$y
...
for (iter in 1:niter) {
  ...
  # add up samples
  sum_beta <- sum_beta + beta
  sum_beta2 <- sum_beta2 + beta^2
  # output current sample
  cat("iteration: ", iter, " -- beta0: ", beta[1], " -- beta1: ", beta[2], "\n")
}
cat("Standard Errors: \n")
print(sqrt((sum_beta2 - sum_beta^2/niter) / (niter-1)))
```
