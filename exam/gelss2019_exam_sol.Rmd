...
\textit{Gegeben ist der folgende Datensatz zu Körpergewicht, Brustumfang und Widerristhöhe. Weiter sind auch die Resultate des Fits des vollen Modells und eines reduzierten Models mit nur Brunstumfang als beschreibende Variable gegeben. Berechnen Sie den Mallow $C_p$-Wert für beide Modelle und begründen Sie, welches der beiden Modelle besser ist.}
\end{enumerate}
...
\textit{Was ist der Grund, dass wir Modellselektion machen? Weshalb können wir nicht einfach immer das volle Modell mit allen erklärenden Variablen zur Anpassung verwenden? Bitte erklären Sie das unterliegende Phänotmen in Ihren eigenen Worten.}
\end{enumerate}
...
\textit{Im Gesamtzuchtwert der Edelschwein Mutterline werden die verschiedenen Merkmalsgruppen `Exterieur`, `Produktion` und `Reproduktion` wie folgt gewichtet.}
...
tbl_met <- tibble::tibble(Offspring = c(1:n_nr_met_obs), 
                          ...
                          Methan = round(mat_obs[,1], 0))

```


The following data set with average amounts of methan gas emission per year for three offspring per bull are given in the following table.
