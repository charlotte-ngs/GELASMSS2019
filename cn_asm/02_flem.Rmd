...
We are using the following dataset as an example on how to use a fixed linear effects model to estimate the genotypic value of the SNP genotypes.
...
mat_geno_snp <- matrix(c(sample(vec_geno_value_coeff, n_nr_animal, prob = c((1-n_prob_snps)^2, 
                       ...
                       replace = TRUE)),
                       nrow = n_nr_snp)
vec_obs_y <- n_inter_cept + crossprod(mat_geno_snp, vec_geno_val) + rnorm(n = n_nr_animal, mean = 0, sd = n_res_sd)
### # mapping the -1,0,1 codes to genotypes
geno_code_map <- tibble::tibble(code = c(-1, 0, 1),
...
geno_code %>% 
  ...
  select(`SNP H`) -> geno_snp_h
geno_snp_all <- bind_cols(geno_snp_g, geno_snp_h)
### # add the data
tbl_obs <- tibble::tibble(Observation = round(vec_obs_y[,1], digits = 0))
...
knitr::kable(tbl_all_data,
             booktabs = TRUE,
             longtable = TRUE,
             caption = "Animals With A Single SNP Locus Affecting A Quantitative Trait",
             escape = FALSE)
```
